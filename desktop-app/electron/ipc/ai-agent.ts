...
async function executeToolCall(
  ...
  switch (toolName) {
    ...
    case 'create_intune_app': {
      // Normalize minimumSupportedWindowsRelease — Claude may pass human-readable strings.
      // Graph beta (api-version 5026+) requires specific enum values. Confirmed working values:
      //   Windows 10: windows10_21H2, windows10_22H2, 2H20, 2004, 1903, 1809, 1607
      //   Windows 11: Windows11_21H2, Windows11_22H2, Windows11_23H2, Windows11_24H2
      const rawMinOs = String(input.minimum_os ?? 'windows10_21H2')
      const MIN_OS_MAP: Record<string, string> = {
        // Human-readable → Graph enum
        ...
        // Old wrong W-prefix values → correct values
      ...
      // setupFilePath = just the filename of the entry-point script (no path, no folder)
      ...
      // Write bodyJson to a temp file to avoid PowerShell 5.1 argument-passing
      // mangling of JSON special characters ({ } : [ ] ") on the command line.
      const tmpJsonPath = path.join(require('os').tmpdir(), `intune-body-${Date.now()}.json`)
...
export function registerAiAgentHandlers(win: BrowserWindow, db: Database): void {
  ...
  ipcMain.handle('ipc:ai:cancel', (_event, req: { jobId: string }) => {
    const job = activeJobs.get(req.jobId)
    if (job) {
      job.abortController.abort()
      activeJobs.delete(req.jobId)
      sendEvent('job:error', { jobId: req.jobId, error: 'Cancelled by user', phase: job.phase })
      return { success: true }
    }
...
Respond ONLY with a JSON array. No markdown, no explanation.`,
  ...
  // Upload-only job (steps 11-12 only: create Intune app record + upload .intunewin)
  // Uses metadata captured during the package-only job — no re-download, no re-packaging
  ipcMain.handle('ipc:ai:upload-only', async (_event, req: {
    intunewinPath: string
  ...
  // Package-only job (steps 1-10: search → download → scripts → build_package; NO upload)
  ipcMain.handle('ipc:ai:package-only', async (_event, req: {
    userRequest: string
    jobId?: string
  }) => {
    const jobId = req.jobId ?? uuidv4()
...
async function runDeployJob(
  ...
  const sourceRoot = getSettingRow('source_root_path', path.join(app.getAppPath(), '..', '..', '..', 'Source'))
  const outputFolder = getSettingRow('output_folder_path', path.join(app.getAppPath(), '..', '..', '..', 'Output'))
...
// Tools available for the package-only pipeline (upload tools intentionally excluded)
...
- Output folder: ${outputFolder}  →  Pass this as output_folder to build_package`
  ...
  while (iterations++ < 20) {
    ...
    for (const block of response.content) {
      ...
      try {
        const result = await executeToolCall(block.name, block.input as Record<string, unknown>, jobId, sendEvent, db)
        // Capture metadata from generate_package_settings (has all app info Claude resolved)
        if (block.name === 'generate_package_settings') {
          capturedPackageSettings = block.input as Record<string, unknown>
        }
        // Capture the intunewin path when build_package finishes
        if (block.name === 'build_package') {
          const r = result as { success?: boolean; intunewinPath?: string }
