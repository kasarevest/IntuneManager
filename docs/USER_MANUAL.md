...
**Version:** 1.6 | **Audience:** IT Administrators | **Last Updated:** 2026-04-10
...
11. [Updating an Existing Application](#11-updating-an-existing-application)
12. [Update All — Batch Updates](#12-update-all--batch-updates)
...
When any of your Intune apps have a newer version available on winget, a count badge appears on the **Installed Apps** button in the top navigation bar. This badge is visible on all pages and is calculated once per session.

### Page structure

IntuneManager has six main pages accessible from the top navigation bar:
...
| **History** | `/history` | Audit trail of all deployments — filter by status, paginated, CSV/JSON export |
| **Audit Log** | `/audit-log` | Expandable deployment record view with full metadata per record |
| **Devices** | `/devices` | Device compliance, Windows Update status, driver updates, diagnostics |

Navigation is standardized across all pages: **Dashboard → Installed Apps → App Catalog → Deploy → History → Devices**
...
|-------|--------------|------------|
| IntuneWinAppUtil Path | Full path to `IntuneWinAppUtil.exe` | ✅ green when path exists on disk |
| Source Root Path | Parent folder for app source subfolders | ✅ green when path exists on disk |
| Output Folder Path | Where `.intunewin` files are saved | ✅ green when path exists on disk |

Each path field shows a live validation icon (✅ exists / ❌ not found) as you type. The **Save** button is disabled while any path shows ❌.
...
Each field shows a ✅ or ❌ icon as you type — green means the path exists, red means it doesn't. The Save button stays disabled until all paths are valid.
...
If the recommendations include apps from 3 or more distinct categories, a **category filter** dropdown appears at the top of the grid. Use it to narrow down recommendations by type (e.g. "Security", "Productivity", "Development").
...
The History page provides a complete audit trail of every deployment IntuneManager has performed.

### Opening the History page

Click **History** in the top navigation bar from any page.

### What it shows

Each row in the table represents one deployment attempt and includes:

| Column | Description |
|--------|-------------|
| Date | When the deployment started |
| App Name | Name of the app that was deployed or updated |
| Version | The version that was deployed |
| Operation | `deploy` (new app) or `update` (existing app) |
| Status | `success`, `failed`, or `running` |
| Duration | How long the deployment took to complete |
| Error | Error message if the deployment failed |

### Filtering

Use the **All / Success / Failed** filter tabs at the top to narrow the list. The record count updates to reflect the current filter.

### Pagination

Records are shown 50 per page. Use **← Prev** and **Next →** to navigate.

### Exporting

1. Choose a format from the dropdown: **CSV** (default) or **JSON**
2. Click **Export** — the browser downloads a file named `deployment-history.csv` or `deployment-history.json`
3. The export includes all records matching the current filter (not just the current page)

> The Export button is disabled when there are no records to export.

### Audit Log

The **Audit Log** page (`/audit-log`, accessible from the History page's nav bar) shows the same deployment data in an expandable list format. Click any row to expand it and see full metadata: job ID, winget ID, Intune app ID, both timestamps, and the complete error message.

---

## 11. Updating an Existing Application
...
4. When one app finishes (success or failure), the next one starts immediately
5. When all apps are processed, a **Summary modal** appears

### Update All Summary modal

After the batch completes, a modal shows:
- Stat cards: **Succeeded / Failed / Skipped** counts
- Per-app table: each app with its final status and error message (if failed)

Click **Close** to dismiss.

### If an app fails

The queue continues when an individual app fails — other apps in the batch are still processed.
- The failed app appears in the Summary modal with the error message
...
The theme applies immediately across the entire application. Your preference is saved to the database and restored on next load.

**Settings → General — Security**

| Setting | Description |
|---------|-------------|
| WinTuner Module Hash | SHA-256 hash of your `WinTuner.psd1` file. When set, IntuneManager verifies the hash before every WinTuner operation. Leave blank to skip verification. |

To get the hash: open PowerShell and run `(Get-FileHash 'C:\path\to\WinTuner.psd1' -Algorithm SHA256).Hash`. Paste the result here.
...
-> Summary modal appears: review succeeded/failed counts -> Close
-> History page: verify deployment records
```

### View deployment history

```
Any page -> History (in top nav) -> Filter by All / Success / Failed
-> Export as CSV or JSON if needed
```
