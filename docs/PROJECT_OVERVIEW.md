...
**Six-page navigation model**
Navigation is standardised across all pages as: Dashboard → Installed Apps → App Catalog → Deploy → History → Devices. Each page has a single, focused responsibility:
...
| App Catalog | `/catalog` | Discovery — AI recommendations (DB-cached, background refresh) + winget search; category filter |
| Deploy | `/deploy` | Execution — ready `.intunewin` packages + active job progress panel; Update All queue |
| History | `/history` | Deployment audit trail — paginated table of all deployments, filter by status, CSV/JSON export |
| Audit Log | `/audit-log` | Expandable deployment record viewer with full metadata (job ID, Intune app ID, error details) |
...
**Version comparison:** Compares the version string in Intune against the latest winget version. Uses the `compareVersions` utility in `src/lib/version.ts`, which handles semver, v-prefix stripping, 8-digit YYYYMMDD date versions (e.g. OneDrive), quad-part versions (e.g. Chrome 132.0.6834.83), and mixed formats. Truly non-comparable formats return `'unknown'`.
...
- **Update All (N)** — queues all outdated apps and runs them sequentially; shows progress badge "2 of 5: Google Chrome". Continues on individual app failure (does not halt queue). After all items are processed, shows a summary modal with per-app success/failure breakdown.

**Update badge:** A count badge appears on the "Installed Apps" nav button on all pages when updates are available, powered by `useUpdateCount` hook with a module-level singleton cache (fires one PS/Graph call per session).
...
**Category filter:** When the recommendation set contains 3 or more distinct categories, a `<select>` dropdown appears in the section header to filter cards by category.
...
| `app_deployments` | Deployment history log — populated on every deploy start/complete/fail. Viewable at `/history` and `/audit-log`. |
...
│   │   │   ├── AppCatalog.tsx          <- Discovery (AI recs cached, winget search, category filter)
│   │   │   ├── Deploy.tsx              <- Execution (ready packages, job progress, queue + summary modal)
│   │   │   ├── DeploymentHistory.tsx   <- Deployment history table (filter, pagination, CSV/JSON export)
│   │   │   ├── AuditLog.tsx            <- Expandable deployment record viewer (full metadata)
...
│   │   │   ├── useRecommendations.ts   <- DB cache + background refresh + session cache
│   │   │   ├── useDeployJob.ts         <- Job subscription management
│   │   │   └── useUpdateCount.ts       <- Module-level singleton: WinTuner update count badge
│   │   ├── components\
│   │   │   ├── AppCard.tsx             <- App card (initials, name, publisher, buttons)
│   │   │   ├── AppCatalogTable.tsx     <- App table (version checking, update badge)
│   │   │   ├── AssignmentModal.tsx     <- Post-deploy group assignment (MRU + all AAD groups)
...
│   │       ├── version.ts              <- compareVersions() — semver, date-based, quad-part, v-prefix
...
│   │   │   ├── settings.ts       <- App settings endpoints + POST /validate-path
│   │   │   ├── ai.ts             <- AI agent endpoints + SSE job streaming
│   │   │   ├── deployments.ts    <- GET /api/deployments (paginated) + GET /api/deployments/export
...
1. **N concurrent winget calls** — `useAppCatalog` fires one `winget.exe` process per Intune app during version checking. With 100+ apps this can be resource-intensive.

2. **No IPC channel whitelist** — `preload.ts` accepts any channel string. Identified in peer review as a security hardening opportunity.

3. **Claude API key encryption uses MachineGuid** — Identified in peer review; DPAPI (`ProtectedData.Protect`) would be stronger. Current implementation is obfuscation, not strong encryption.

4. **Driver update status always 'unknown'** — Graph API `managedDevices` does not expose a dedicated driver update status field. The Devices page shows 'Unknown' for all devices.

5. **Device diagnostics shows 'Request Logs' regardless of prior requests** — No polling of existing log collection request status; the button always triggers a new request.

6. **Web mode: `IntuneWinAppUtil.exe` unavailable** — The packaging binary is Windows-only and is not included in the Docker image. The `build_package` AI agent tool will fail in the web container. Resolution: will spawn an Azure Container Instance (Windows, pay-per-second) on demand for packaging jobs.

7. **Web mode: in-memory SSEManager** — `SSEManager` holds active SSE connections in process memory. If Container Apps scales to more than one replica, SSE events will only reach clients connected to the same replica. Resolution: future work will use Azure Service Bus for fan-out.

8. **SQL Server in West US 2, everything else in East US** — Introduced cross-region latency (~30 ms). Azure SQL Serverless had no quota in East US or East US 2 on this subscription at provisioning time.
...
### Phase 4 Resolved Issues *(2026-04-10)*

- **No AAD group assignment after deploy** — Fixed: `AssignmentModal` shown automatically after each successful deployment. Searches all AAD security groups (with MRU at top), per-group intent toggle (Required/Available), single `/assign` batch POST to Graph API. `GroupAssignmentHistory` Prisma model tracks recently-used groups.

- **PowerShell scripts could hang indefinitely** — Fixed: `runPsScript()` in both `electron/ipc/ps-bridge.ts` and `server/services/ps-bridge.ts` now accept `timeoutMs` parameter. `SCRIPT_TIMEOUTS` constants defined per script type (search=60s, download=300s, build=180s, upload=600s). All 7 tool calls in `executeToolCall` pass per-script timeouts.

- **No deployment history persistence or UI** — Fixed: `app_deployments` table populated on every deploy start (`status='running'`), completion (`status='success'`), and failure (`status='failed'`). Two new pages: `/history` (paginated table, status filter, CSV/JSON export) and `/audit-log` (expandable row view with full metadata).

- **Non-semver version comparison returned 'unknown' for date/quad-part versions** — Fixed: `compareVersions` extracted to `src/lib/version.ts`. Handles v-prefix, suffix stripping, 8-digit YYYYMMDD date versions, quad-part (e.g. 132.0.6834.83), mixed triple/quad, mixed date/semver → `unknown`. 20 vitest test cases.

- **AI-provided paths not validated (path traversal risk)** — Fixed: `validatePathInBase()` utility in `server/utils/path-validator.ts`. All 4 `generate_*` tool cases validate `source_folder` before any `fs` operation. UNC paths, `..` traversal, and paths outside `sourceRoot` rejected.

- **Orphaned jobs after server restart** — Fixed: On server startup, all `app_deployments` rows with `status='running'` are updated to `status='failed'` with message "Server restarted while job was running."

- **Update All stopped on first app failure** — Fixed: `onJobError` in queue mode now advances the queue rather than halting. All queued apps process regardless of individual failures.

- **No update summary after Update All** — Fixed: Summary modal shown after all queue items complete. Shows stat cards (succeeded / failed / skipped) and a per-app table with status and error messages.

- **Settings path validation only caught on job run** — Fixed: Settings → Paths now shows live ✅/❌ icons next to IntuneWinAppUtil, Source Root, and Output Folder fields (debounced 600 ms, `POST /api/settings/validate-path`). Save button disabled when any path is invalid.

- **No dark/light theme toggle** — Fixed: Settings → General now has a Dark/Light theme toggle. Selection applied immediately via `document.documentElement.dataset.theme` and persisted to `app_settings`.

- **No update count badge on navigation** — Fixed: `useUpdateCount` hook with module-level singleton cache shows a count badge on the "Installed Apps" nav button across all pages.

- **App Catalog had no category filtering** — Fixed: When recommendations include 3+ distinct categories, a category filter dropdown appears in the section header.

- **WinTuner module could be tampered (supply chain)** — Fixed: SHA-256 hash of `WinTuner.psd1` configurable in Settings → General → Security. `New-WtPackage.ps1`, `Deploy-WtApp.ps1`, and `Update-WtApp.ps1` verify the hash before `Import-Module WinTuner`.
