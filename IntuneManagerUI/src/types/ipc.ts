...
export interface WtUpdateItem {
  ...
  graphId: string
  daysKnown?: number           // days since this (packageId, latestVersion) was first detected
  autoUpdateEligible?: boolean // true when daysKnown > 7
}
...
export interface WtUpdateAppReq { packageId: string; graphId: string; packageFolder: string; version?: string; jobId?: string; updateType?: 'auto' | 'manual'; currentVersion?: string; latestVersion?: string }
